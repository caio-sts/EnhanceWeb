...
def enhance(origPath: str, storePath: str, method: str):
    ...
    print(f'Showing progress inside folder:')
    for image in os.listdir(origPath):
        if image[:-3] == "png" or "jpg" or "tif":
            imgArray =  mpimg.imread(origPath+"/"+image)

            if imgArray.shape[2] == 4:            
                imgPIL = Image.open(origPath+"/"+image)
                imgArray = imgPIL.convert('RGB')

            img = enhanceApplier(np.array(imgArray))
            mpimg.imsave(storePath+"/"+str(numberImagesEnhanced)+" "+image[:-3]+"png", img)
            print(numberImagesEnhanced)
            ...
            if image[:-3] == "png" or "jpg" or "tif":
                for i in range(numberOfGenerations):
                    ...
                    if operation == 0 and paramShear == 0:
                        ...
                        mpimg.imsave(storePath+"/"+str(i)+" "+image[:-3]+"png", img)
                        ...
                        if len(image.split('.')[-1]) == 4:
                            mpimg.imsave(storePath+"/"+str(i)+" "+image[:-4]+"png", (255*img_shear).astype(np.uint8))
                        else:
                            mpimg.imsave(storePath+"/"+str(i)+" "+image[:-3]+"png", (255*img_shear).astype(np.uint8))
                        ...
                        if len(image.split('.')[-1]) == 4:
                            mpimg.imsave(storePath+"/"+str(i)+" "+image[:-4]+"png", (255*img_rotat).astype(np.uint8))
                        else:
                            mpimg.imsave(storePath+"/"+str(i)+" "+image[:-3]+"png", (255*img_rotat).astype(np.uint8))
                        ...
                        if len(image.split('.')[-1]) == 4:
                            mpimg.imsave(storePath+"/"+str(i)+" "+image[:-4]+"png", (255*img_transl).astype(np.uint8))
                        else:
                            mpimg.imsave(storePath+"/"+str(i)+" "+image[:-3]+"png", (255*img_transl).astype(np.uint8))
